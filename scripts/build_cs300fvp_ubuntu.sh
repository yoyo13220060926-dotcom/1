...
CS300FVP_REPO="${CS300FVP_REPO:-https://github.com/heslabs/cs300fvp}"
CS300FVP_BRANCH="${CS300FVP_BRANCH:-main}"
CS300FVP_REF="${CS300FVP_REF:-}"
CS300FVP_LOCAL_SRC="${CS300FVP_LOCAL_SRC:-}"
VERIFY_ONLY="${VERIFY_ONLY:-0}"

SRC_DIR="/work/src/cs300fvp"
OUT_DIR="/work/out"

mkdir -p /work/src "$OUT_DIR"

copy_or_clone_source() {
  if [[ -n "$CS300FVP_LOCAL_SRC" ]]; then
    echo "[INFO] Using local source from: $CS300FVP_LOCAL_SRC"
    rm -rf "$SRC_DIR"
    cp -a "$CS300FVP_LOCAL_SRC" "$SRC_DIR"
    return
  fi

  if [[ ! -d "$SRC_DIR/.git" ]]; then
    echo "[INFO] Cloning $CS300FVP_REPO (branch: $CS300FVP_BRANCH)"
    git clone --depth 1 --branch "$CS300FVP_BRANCH" "$CS300FVP_REPO" "$SRC_DIR"
  else
    echo "[INFO] Reusing existing source: $SRC_DIR"
    git -C "$SRC_DIR" fetch --depth 1 origin "$CS300FVP_BRANCH"
    git -C "$SRC_DIR" checkout -f "$CS300FVP_BRANCH"
    git -C "$SRC_DIR" reset --hard "origin/$CS300FVP_BRANCH"
  fi

  if [[ -n "$CS300FVP_REF" ]]; then
    echo "[INFO] Checking out ref: $CS300FVP_REF"
    git -C "$SRC_DIR" fetch --depth 1 origin "$CS300FVP_REF" || true
    git -C "$SRC_DIR" checkout -f "$CS300FVP_REF"
  fi
}
...
run_build() {
  ...
  if [[ -x ./build.sh ]]; then
    ...
    echo "[ERROR] Please update scripts/build_cs300fvp_ubuntu.sh to match upstream layout."
    exit 1
  fi
}
