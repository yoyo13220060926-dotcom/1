...
IMAGE_NAME="${IMAGE_NAME:-cs300fvp-ubuntu-builder}"
...
# You may pin ref for deterministic output by setting CS300FVP_REF env before running this script.
docker_args=(
  --rm -it
  -e "CS300FVP_REPO=${CS300FVP_REPO:-https://github.com/heslabs/cs300fvp}"
  -e "CS300FVP_BRANCH=${CS300FVP_BRANCH:-main}"
  -e "CS300FVP_REF=${CS300FVP_REF:-}"
  -e "CS300FVP_LOCAL_SRC=${CS300FVP_LOCAL_SRC:-}"
  -e "VERIFY_ONLY=${VERIFY_ONLY:-0}"
  -v "$OUT_DIR:/work/out"
)

if [[ -n "${CS300FVP_LOCAL_SRC:-}" ]]; then
  docker_args+=( -v "$CS300FVP_LOCAL_SRC:$CS300FVP_LOCAL_SRC:ro" )
fi
